...
const authHandler = async (req: Request, res: Response, next: NextFunction) => {
	...
	if (!pin) {
		throw new AppError('PIN is required in X-User-PIN header.', Status.BAD_REQUEST);
	}

	const user = await getPinStatus();
	if (!user.data.exists) {
		throw new AppError('PIN is not set. Please set the PIN first.', Status.BAD_REQUEST);
	}
	await verifyPin(pin);
	next();
};
